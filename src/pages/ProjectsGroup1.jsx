...
const group1Projects = projects.slice(0, 3);
                                    ...
                                    <div className="space-y-0 border-t border-slate-100 dark:border-slate-800/60">
                                        {project.scope.map((item, i) => (
                                            <div key={i} className="group flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800/60 cursor-pointer">
                                                ...
                                                <ArrowRight className="w-4 h-4 text-blue-600 opacity-100 md:opacity-0 md:group-hover:opacity-100 -translate-x-4 md:group-hover:translate-x-0 transition-all duration-300" />
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="pt-6">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {project.tech.map((t, i) => (
                                                <span key={i} className="px-3 py-1.5 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-lg text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all cursor-default">
                                                    {t}
                                                </span>
                                            ))}
                                        </div>
                            ...
                            <FadeUp delay={0.1} className="lg:col-span-5 order-2 lg:order-1">
                                <div className="space-y-6">
                                    ...
                                    <div className="pt-6">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {project.tech.map((t, i) => (
                                                <span key={i} className="px-3 py-1.5 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-lg text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 hover:bg-blue-600 hover:text-white transition-all cursor-default">
                                                    {t}
                                                </span>
                                            ))}
                                        </div>
...
export default function ProjectsGroup1() {
    return (
        <article className="relative bg-white dark:bg-[#0a0f1e] text-slate-900 dark:text-white font-sans overflow-x-hidden transition-colors duration-300">
            <main className="relative z-10 bg-white dark:bg-[#0a0f1e] pt-20">
                {group1Projects.map((project, index) => (
                ...
                <section className="py-40 text-center border-t border-slate-100 dark:border-slate-800">
                    <FadeUp>
                        <h3 className="text-xl md:text-3xl font-black uppercase tracking-tight text-slate-900 dark:text-white mb-8">
                            Transforming India's <br />Digital Landscape
                        </h3>
